...
async function readProgress(root) {
    ...
    return txt
        ...
        .filter((j) => !!j && typeof j.ts === "string" && typeof j.percent === "number");
}
async function readTodosCount(root) {
    const { todo } = await ensureLocalScaffold(root);
    const arr = await fs.readJSON(todo).catch(() => []);
    return arr.filter((t) => !t.doneAt).length;
}
...
export async function metricsPredict() {
    ...
    const prog = await readProgress(root);
    if (prog.length < 2) {
        const payload = { computedAt: new Date().toISOString(), currentPercent: prog[0]?.percent ?? 0, dailyVelocity: 0, etaDays: null, etaDate: null };
...
function parseEstimateToHours(s) {
    if (!s)
        return null;
...
export async function metricsTickets() {
    ...
    const rows = ["id,type,index,name,createdAt,closedAt,duration_hours,estimate,estimate_hours,variance_hours"];
    for (const id of ids) {
        try {
