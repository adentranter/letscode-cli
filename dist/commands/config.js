...
import { ensureGlobalScaffold } from "../lib/paths.js";
...
async function pickEditor() {
    ...
    const plat = platform();
    if (plat === "mac")
        return { bin: "open", args: ["-t"] };
    if (plat === "win")
        return { bin: "notepad", args: [] };
    // linux default
    if (await has("nano"))
        return { bin: "nano", args: [] };
    return { bin: "vi", args: [] };
}
export async function cmdConfigEdit() {
    ...
    if (!(await fs.pathExists(cfg)))
        await fs.outputJSON(cfg, { createdAt: new Date().toISOString() }, { spaces: 2 });
