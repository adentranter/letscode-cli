...
function parseIntervalToMs(input) {
    if (typeof input === "number" && Number.isFinite(input))
        return Math.max(10_000, input * 1000);
...
export async function cmdWatch(opts = {}) {
    ...
    const ignored = [
        (p) => p.includes(`${path.sep}.git${path.sep}`),
        (p) => p.includes(`${path.sep}node_modules${path.sep}`),
        (p) => p.includes(`${path.sep}${LOCAL_DIR}${path.sep}`),
    ];
    const watcher = chokidar.watch(root, {
        ignoreInitial: true,
        persistent: true,
        depth: 99,
        ignored: (p) => ignored.some(fn => fn(p)),
    });
    const track = (file) => {
        const r = rel(file);
        if (!r || r.startsWith(".."))
    ...
    const tick = async () => {
        ...
        const msg = (ans.message || "").trim();
        const pct = typeof ans.progress === "number" ? ans.progress : undefined;
