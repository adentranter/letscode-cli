...
function injectBetweenMarkers(content, start, end, injection) {
...
export async function cmdRetake(opts = {}) {
    ...
    const md = [
        ...
        ``,
        `## Tech stack`,
        ...(Array.isArray(baseline.techStack) ? baseline.techStack.map((t) => `- ${t}`) : []),
        ``,
        `## Key files`,
        ...(Array.isArray(baseline.keyFiles) ? baseline.keyFiles.map((t) => `- ${t}`) : []),
    ].join("\n");
