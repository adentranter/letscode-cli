...
async function loadTodos() {
    ...
    const items = await readJson(todo, []);
    return { file: todo, items };
}
export async function cmdTodoAdd(title, opts) {
    if (!title || !title.trim())
    ...
    const files = (opts.files || "")
        ...
        .map((s) => s.trim())
        .filter(Boolean);
    const next = { title: title.trim(), createdAt: new Date().toISOString(), ...(files.length ? { files } : {}) };
    items.push(next);
...
export async function cmdTodoDone(indexStr) {
    const idx = parseInt(indexStr, 10);
    if (!Number.isFinite(idx) || idx < 1)
        throw new Error("Provide a valid index (1-based)");
    const { file, items } = await loadTodos();
    if (idx > items.length)
...
export async function cmdTodoRemove(indexStr) {
    const idx = parseInt(indexStr, 10);
    if (!Number.isFinite(idx) || idx < 1)
        throw new Error("Provide a valid index (1-based)");
    const { file, items } = await loadTodos();
