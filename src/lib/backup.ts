...
function repoId(absPath: string) {
...
export async function backupSync() {
  ...
  await fs.copy(local, dest, {
    ...
    filter: (src: string) => {
      ...
      if (base.endsWith(".tmp") || base.endsWith("~")) return false;
      return true;
    },
  });
...
export async function backupWatch() {
  ...
  const copyOne = async (srcPath: string) => {
  ...
  const removeOne = async (srcPath: string) => {
    const rel = path.relative(local, srcPath);
    const out = path.join(dest, rel);
    await fs.remove(out);
  };
...
export async function backupRestore(force = false) {
  ...
  if (!(await fs.pathExists(src))) throw new Error("No backup found for this repo.");
  if (!force) throw new Error("Refusing to overwrite local .letscode without --force");
