...
export async function ensureLocalScaffold(root: string) {
  ...
  if (!(await fs.pathExists(events)))   await fs.outputFile(events, JSON.stringify({ type: "header", version: 1, created_at: new Date().toISOString() }) + "\n");
...
export async function ensureGlobalScaffold() {
  ...
  if (!(await fs.pathExists(events))) await fs.outputFile(events, JSON.stringify({ type: "header", version: 1, created_at: new Date().toISOString() }) + "\n");
  if (!(await fs.pathExists(repos)))  await fs.outputFile(repos, "[]");
