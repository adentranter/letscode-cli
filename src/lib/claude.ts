...
export async function ensureClaudeInstalled(interactive = true) {
  if (await has("claude")) return true;

  if (!interactive) return false;

  const plat = platform();
  const methods: { title: string; value: "brew"|"choco"|"winget"|"skip" }[] = [];
  if (plat === "mac") methods.push({ title: "Install via Homebrew (if available)", value: "brew" });
  if (plat === "win") methods.push({ title: "Install via winget (if available)", value: "winget" }, { title: "Install via Chocolatey", value: "choco" });
  if (plat === "linux") methods.push({ title: "Skip (install manually, then re-run lc doctor)", value: "skip" });
...
export async function ensureClaudeRepoAccess() {
  ...
  let existing: any = {};
  try { existing = await fs.readJSON(cfg); } catch {}
