...
export async function impactScan() {
  ...
  const res: ScanResult = { prismaModels, sqlTables, openapiPaths, hotFiles };
...
export async function impactSet(opts: { tables?: string; apis?: string; files?: string }) {
  const root = await repoRoot();
  await ensureLocalScaffold(root);
  const ticket = await currentTicket();
  if (!ticket) throw new Error("Not on a ticket branch");
