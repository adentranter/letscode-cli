...
export async function createTicket(kind: "feature"|"bug", rawName: string, withReadme = false, interactive = false) {
  ...
  if (interactive) {
    ...
    const meta = {
      ...
      goal: ans.goal || "",
      acceptance: Array.isArray(ans.acceptance) ? ans.acceptance.map((s:string)=>s.trim()).filter(Boolean) : [],
      estimate: ans.estimate || "",
      stakeholders: Array.isArray(ans.stakeholders) ? ans.stakeholders.map((s:string)=>s.trim()).filter(Boolean) : [],
      risks: Array.isArray(ans.risks) ? ans.risks.map((s:string)=>s.trim()).filter(Boolean) : [],
    };
    ...
    try { await execa("git", ["add", rd, path.join(tdir, "ticket.json")]); await execa("git", ["commit", "-m", `docs(${kind}): seed ${branch} (metadata)`]); } catch {}
  }
