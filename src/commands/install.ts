...
export async function cmdInstall() {
  ...
  try { await execa("git", ["--version"]); } catch { throw new Error("git is required; please install git first."); }

  const root = await repoRoot();
  let inRepo = true;
  try { await execa("git", ["rev-parse", "--git-dir"], { cwd: root }); } catch { inRepo = false; }
