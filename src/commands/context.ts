...
type ContextOpts = { stdout?: boolean };

function parseLastNonEmptyLine(s: string): any | null {
  const lines = s.trim().split(/\r?\n/).filter(Boolean);
  if (!lines.length) return null;
  try { return JSON.parse(lines[lines.length - 1]); } catch { return null; }
}

export async function cmdContext(opts: ContextOpts = { stdout: false }) {
  ...
  if (ticket) {
    ...
    try {
      ...
      ticketUpdatesTail = tail.map((l) => { try { return JSON.parse(l); } catch { return null; } }).filter(Boolean);
    } catch {}
  }
